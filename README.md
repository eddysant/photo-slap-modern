...
- **Safe delete** — files are moved to the system Trash, never hard-deleted, and every delete can be taken back: the file leaves the slideshow at once but the trashing is held for a few seconds while the toast offers **Undo**.
...
- **Keyboard shortcuts** — `←`/`→` previous/next, `Space` play/pause, `G` grid, `P` photo frame, `H` favorite, `T` tags, `K`/`Enter` culling keep, `X` culling reject, `F` reveal in Finder, `M`/`N` video ±10 seconds, `1`–`3` quick-move, `Delete`/`Backspace` delete, `Esc` close overlays. Press **`?`** for the full list on screen; they are also in the **Actions** menu in the menu bar.

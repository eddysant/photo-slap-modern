...
- **Fonts are self-hosted** (`public/fonts/silkscreen-*.woff2`, `@font-face` in `index.html`). Google Fonts was a launch-time network dependency on an app meant to run offline as a photo frame, and it would have forced two external origins into the CSP.

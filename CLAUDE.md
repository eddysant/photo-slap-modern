...
4. **Extracting the slideshow engine** — playback, the stall watchdog and the preloader are hooks now, but `App.tsx` still owns index/direction/video-ref state directly, so those parts have no unit coverage (only e2e).
5. **Symlink escapes** — the allowlist compares resolved paths but does not `realpath`, so a symlink *inside* an opened folder can still point outside it. Low risk (the user opened the folder), but it is the remaining gap in `pathAccess`.

...
- **Culling mode**: persisted setting, forces photos-only paused review when enabled from the UI. The always-visible review bar maps `K`/`Enter` to keep-and-next and `X` to safe Trash rejection; existing `H` and `1`–`3` favorite/quick-move shortcuts remain available.
- **No-repeat shuffle**: `shuffleHistory.ts` orders unseen paths before seen paths and resets only after the eligible set completes. `App` persists `shuffleHistory` in electron-store keyed by sorted root set + media filter and records a slide as soon as it is displayed.
- `components/LibraryHealthModal.tsx` + `electron/libraryHealth.ts` — filterable health report. Main-process workers validate image decoding (including HEIC), video signatures, tiny dimensions/size, embedded capture dates, common unsupported media extensions, and missing paths referenced by merged sidecars.
